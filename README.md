...
The function has several optional arguments, which allow to specify *e.g.* regression terms, correlation lengths, prior variance and nugget, etc. If any is not provided, a default choice is made. The script `Emulation_Example.R` illustrates of use of the function on a toy problem, with both default and tailored choices. Plots illustrating emulator performance are below.
